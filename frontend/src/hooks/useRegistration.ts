...
export const useRegistration = () => {
  ...
  const registerStep1 = useCallback(async (email: string, password: string) => {
    try {
  ...
  const registerStep2 = useCallback(async (name: string, username: string) => {
    try {
      dispatch(setRegistrationState({ isLoading: true, error: null }));
  ...
  const registerStep3 = useCallback(async (data: {
    avatar?: string;
    bio?: string;
