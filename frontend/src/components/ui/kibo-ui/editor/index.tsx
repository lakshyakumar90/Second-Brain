...
// import { Separator } from '@/components/ui/separator'; // Unused due to bubble menu being disabled
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export const EditorProvider = ({
  ...
  const defaultExtensions = [
    ...
    Table.configure({
      HTMLAttributes: {
        class: cn(
          'relative m-0 mx-auto my-3 w-full max-w-full table-auto border-collapse overflow-visible rounded-none text-sm'
        ),
      },
  ...
  return (
    <TooltipProvider>
      <div className={cn(
        ...
        // ProseMirror container constraints
        '[&_.ProseMirror]:w-full [&_.ProseMirror]:max-w-full [&_.ProseMirror]:overflow-x-auto',
        '[&_.ProseMirror]:break-words [&_.ProseMirror]:word-wrap',
        // Heading styles
        '[&_h1]:text-3xl [&_h1]:font-bold [&_h1]:mt-6 [&_h1]:mb-4',
        '[&_h2]:text-2xl [&_h2]:font-bold [&_h2]:mt-5 [&_h2]:mb-3',
        '[&_h3]:text-xl [&_h3]:font-bold [&_h3]:mt-4 [&_h3]:mb-3',
        '[&_h4]:text-lg [&_h4]:font-bold [&_h4]:mt-4 [&_h4]:mb-2',
        '[&_h5]:text-base [&_h5]:font-bold [&_h5]:mt-3 [&_h5]:mb-2',
        '[&_h6]:text-sm [&_h6]:font-bold [&_h6]:mt-3 [&_h6]:mb-2',
        // Table responsive handling
        '[&_table]:w-full [&_table]:max-w-full [&_table]:table-auto',
        '[&_table]:relative [&_table]:z-0',
        // Table cell and row hover effects for better interaction
        '[&_td]:relative [&_th]:relative',
        // Ensure table menu controls are properly positioned
        'relative'
      )}>
...
export const EditorFloatingMenu = ({
  ...
  // Floating menu temporarily disabled due to Tiptap v3 compatibility
  return (
...
export const EditorBubbleMenu = ({
  ...
  // Bubble menu temporarily disabled due to Tiptap v3 compatibility
  return (
    <div className={cn('hidden', className)}>
      {children}
    </div>
...
export const EditorTableGlobalMenu = ({
  ...
  return (
    <div
      className={cn(
        '-translate-x-1/2 flex translate-y-1/2 items-center rounded-full border bg-background shadow-xl z-[9999] overflow-visible',
        {
          hidden: !(left && top),
        }
      )}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        style={{ top, left, position: 'fixed' }}
        className={cn(
          '-translate-x-1/2 -translate-y-1/2 flex h-4 w-7 overflow-visible rounded-md border bg-background shadow-xl z-[9999]',
          {
            hidden: !(left && top),
          }
        )}
      >
        <Button size="icon" variant="ghost" className="h-4 w-7 p-0">
          <EllipsisIcon className="text-muted-foreground" size={12} />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="z-[9999]">{children}</DropdownMenuContent>
    </DropdownMenu>
  );
};
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          className={cn(
            '-translate-x-1/2 -translate-y-1/2 flex h-7 w-4 overflow-visible rounded-md border bg-background shadow-xl z-[9999] p-0',
            {
              hidden: !(left && top),
            }
          )}
          ...
          variant="ghost"
        >
          <EllipsisVerticalIcon className="text-muted-foreground" size={10} />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="z-[9999]">{children}</DropdownMenuContent>
    </DropdownMenu>
